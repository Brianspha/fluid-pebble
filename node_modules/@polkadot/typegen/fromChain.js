...
function generate(metaHex, pkg, output, isStrict) {
  ...
  ? {
    [pkg]: require(path.join(base, 'definitions'))
  } : {};
  ...
  writeFile(path.join(base, 'augment-api.ts'), () => [HEADER('chain'), ...['@polkadot/api/augment/rpc', ...['consts', 'errors', 'events', 'query', 'tx', 'rpc'].filter(key => !!key).map(key => `./augment-api-${key}`)].map(path => `import '${path}';\n`)].join(''));
  process.exit(0);
}
