...
contract FluidPebbleERC20Token is IERC20, Context, Initializable {
    ...
    *@dev init contract variables
    ...
    @param decimals number of decimals the token will be measured it
    @param initialSupply the initial token supply to be minted
     */

function init(string memory name, string memory symbol, uint8 decimals, uint256 initialSupply) public initializer onlyOwner{
        _name = name;
        _symbol = symbol;
        _decimals = decimals;
        _owner = msg.sender;
