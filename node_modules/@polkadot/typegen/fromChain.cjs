...
function generate(metaHex, pkg, output, isStrict) {
  ...
  const base = _path.default.join(process.cwd(), output);

  const extraTypes = pkg // eslint-disable-next-line @typescript-eslint/no-var-requires
  ? {
    [pkg]: require(_path.default.join(base, 'definitions'))
  } : {};
  ...
  try {
    customLookupDefinitions = {
      ...
      types: require(_path.default.join(base, 'lookup.ts')).default
    };
  } catch (error) {
    console.log('No custom definitions found.');
  }

  (0, _index.generateDefaultConsts)(_path.default.join(base, 'augment-api-consts.ts'), metaHex, extraTypes, isStrict, customLookupDefinitions);
  (0, _index.generateDefaultErrors)(_path.default.join(base, 'augment-api-errors.ts'), metaHex, extraTypes, isStrict);
  (0, _index.generateDefaultEvents)(_path.default.join(base, 'augment-api-events.ts'), metaHex, extraTypes, isStrict, customLookupDefinitions);
  (0, _index.generateDefaultQuery)(_path.default.join(base, 'augment-api-query.ts'), metaHex, extraTypes, isStrict, customLookupDefinitions);
  (0, _index.generateDefaultRpc)(_path.default.join(base, 'augment-api-rpc.ts'), extraTypes);
  (0, _index.generateDefaultTx)(_path.default.join(base, 'augment-api-tx.ts'), metaHex, extraTypes, isStrict, customLookupDefinitions);
  (0, _index2.writeFile)(_path.default.join(base, 'augment-api.ts'), () => [(0, _index2.HEADER)('chain'), ...['@polkadot/api/augment/rpc', ...['consts', 'errors', 'events', 'query', 'tx', 'rpc'].filter(key => !!key).map(key => `./augment-api-${key}`)].map(path => `import '${path}';\n`)].join(''));
  process.exit(0);
}
